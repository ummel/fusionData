...
Importantly, `/survey-processed` also contains `.fst` files with the processed microdata. Example: `/survey-processed/RECS/2015/RECS_2015_H_processed.fst`. These files are stored remotely. Use of [.fst files](http://www.fstpackage.org/) allows this data to be read very quickly from disk, in part or in full. The `*_processed.fst` files can be added to a user's local fusionData directory by calling `getSurveyProcessed()`.
