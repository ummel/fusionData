...
You may be prompted to enter the password for the Google Drive account storing the remote files.
...
The number of observations in the donor microdata is now higher, reflecting the use of `implicates = 5`.
...
At this point, we are ready to fuse. This is straightforward using the `train()` and `fuse()` functions from the [fusionModel package](https://github.com/ummel/fusionmodel).
