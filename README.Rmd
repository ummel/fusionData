...
In practice, we sometimes need to "do fusion" in a secure server environment. Since installing the complete fusionData package just to access `fusionOutput()` is overkill (and a pain), a copy of `fusionOutput()` is silently exported in the fusion*Model* package. This allows someone to install and load only the fusionModel package in a server environment and then use `fusionOutput()` to do the fusion step -- assuming the server has access to the necessary input files.
