...
out.path <- file.path(sub("input", "output", dir), sub("train_UP.fst$", "", list.files(dir, "train_UP\\.fst$")))
...
train.data <- fst::read_fst(file.path(dir, list.files(dir, "train_UP\\.fst$")))

# Load results of prepXY() with fusion and predictor variable details
prep <- readRDS(file.path(dir, list.files(dir, "prep_UP\\.rds$")))
...
fsn.path <- train(data = train.data,
                  y = prep$y,
                  x = prep$x,
                  fsn = paste0(out.path, "model_UP.fsn"),
                  weight = "weight",
                  cores = ncores,
                  hyper = hyper.params)
...
# data.table::setDTthr                                                                                                                                                                                                                                                                                                                                                                                                                                           eads(num.cores)
...
# Fuse multiple implicates to training data for internal validation analysis
 valid <- fuse(data = train.data,
              fsn = fsn.path,
              M = nimps,
              fsd = paste0(out.path, "valid_UP.fsd"),
              cores = ncores)
...
saveRDS(validation, file = paste0(out.path, "validation_ornl.rds"))

# Clean up
rm(train.data, prep, valid, validation)

#------------------ Fuse - simulation ------------------------------------------
 # fsn.path = "C:/Users/ka626/Documents/fusionData/fusion/AHS/2019/2019/output/AHS_2019_2019_model_UP.fsn"

  # Load the prediction data
  predict.data <- fst::read_fst(file.path(dir, list.files(dir, "predict_UP\\.fst$")))
  
  # Fuse multiple implicates to ACS and save results to disk as compressed .csv
  fuse(data = predict.data,
       fsn = fsn.path,
       M = nimps,
       fsd = paste0(out.path, "fused_UP.fsd"),
       cores = ncores)
