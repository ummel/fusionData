...
# Assemble final output for householdfile
...
h.final <- d1 %>%
  ...
  select(hid,  weight, everything())

# CreateDictionary() needs label for variables
labelled::var_label(h.final$hid) <- "Unique household identifier"
labelled::var_label(h.final$perrp) <- "RELATIONSHIP TO REFERENCE"
labelled::var_label(h.final$hrfs12m3) <- "Food Security Raw Score, 12-Month Recall"
labelled::var_label(h.final$hulangcode) <- "Census region" 
labelled::var_label(h.final$pwvetwgt) <- "Veteran's weight" 
labelled::var_label(h.final$weight) <- "Food Supplement Weight" 

# Create dictionary and save to disk
dictionary <- createDictionary(data = h.final, survey = "CPS", vintage = 2019, respondent = "H")
saveRDS(object = dictionary, file = "survey-processed/CPS/2019/CPS_2019_H_dictionary.rds")

#----------------
compileDictionary()

# Save data to disk (.fst)
fst::write_fst(x = h.final, path = "survey-processed/CPS/2019/CPS_2019_H_processed.fst", compress = 100)
