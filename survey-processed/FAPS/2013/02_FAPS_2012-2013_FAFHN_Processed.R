...
#######################
d_0 <- read.csv('survey-raw/FAPS/2013/faps_fafhnutrient_puf.csv')  
hei_g <- haven::read_dta('survey-raw/FAPS/2013/faps_eiq_fafhitem_puf.dta') %>% select(-imp_gram_flag)
...
codebook <- read_excel('survey-raw/FAPS/2013/10_FAFH Nutrient Codebook PUF.xlsx', skip = 16) %>%
...
d <- d %>%
  mutate(
    ...
    TOT_ENG = (ENERGY * HEI_GRAMS)/100,  #Fat Ratio
...
fst::write_fst(x = h.final, path = "survey-processed/FAPS/2013/FAPS_2013_FAFHN_processed.fst", compress = 100)
fst::write_fst(x = codebook, path = "survey-processed/FAPS/2013/FAPS_2013_FAFHN_codebook.fst", compress = 100)


##Summarize nutrient variables at the household level#######
h.sum <- d  %>%
  group_by(HHNUM) %>%
  summarise(
  ...
  #  FAT_RATIO = (sum(as.numeric(POLYFAT)) + sum(as.numeric(MONOFAT)))/sum(as.numeric(SATFAT)),
    ...
    HEI_GRAMS = sum((HEI_GRAMS))
  ) %>% ungroup() #%>% mutate(FAT_RATIO = ifelse(SATFAT == 0, 0,FAT_RATIO))

codebook_fafh <- codebook %>% filter(var %in% (intersect(colnames(h.sum), codebook$var))) %>%
  filter(var != 'HHNUM') %>% ungroup() %>%
  mutate(desc = paste0(desc," away from home"))   %>%
  
  add_row(.,var = 'F_WHOLE_CUPS', desc = "Total only whole fruit cups away from home",label = NA, value = NA) %>%
  add_row(.,var = 'F_TOTAL_CUPS', desc = "Total all fruit cups away from home",label = NA, value = NA)  %>%
  add_row(.,var = 'V_TOTAL_CUPS', desc = "Total vegetable cups away from home",label = NA, value = NA)  %>%
  add_row(.,var = 'V_DRKGR_LEG_CUPS', desc = "Total dark green vegetables and legumes cups away from home",label = NA, value = NA)  %>%
  add_row(.,var = 'G_WHOLE_OZ', desc = "Total whole grain (oz) away from home",label = NA, value = NA)  %>%
  add_row(.,var = 'D_TOTAL_CUPS', desc = "Total dairy cups away from home",label = NA, value = NA)  %>%
  add_row(.,var = 'PROTEIN_OZ', desc = "Total protein (oz) away from home",label = NA, value = NA)  %>%
  add_row(.,var = 'PF_SEAFOOD_OZ', desc = "Total seafood and plant protein cups away from home",label = NA, value = NA)  %>%
  add_row(.,var = 'TOT_ENG', desc = "Total energy (kcal) away from home ",label = NA, value = NA)  %>%
  add_row(.,var = 'G_REFINED_OZ ', desc = "Total refined grains (oz) away from home",label = NA, value = NA)  %>%
  add_row(.,var = 'SODIUM_G', desc = "Total sodium (g) away from home ",label = NA, value = NA)  %>%
  add_row(.,var = 'SUGAR_ENG', desc = "Total added sugar energy (kcal) away from home",label = NA, value = NA)  %>%
  add_row(.,var = 'SATFAT_ENG', desc = "Total saturated fat energy (kcal) away from home",label = NA, value = NA)  %>%
  add_row(.,var = 'HEI_GRAMS', desc = "Total food away from home",label = NA, value = NA)  %>%
  
#  add_row(.,var = 'FAT_RATIO', desc = "Fat ratio away from home",label= NA, value = NA)  %>%
  add_row(.,var = 'G_REFINED_OZ', desc = "Refined grains away from home",label= NA, value = NA)  %>%
  add_row(.,var = 'SODIUM_G', desc = "Soidum content away from home",label= NA, value = NA)  %>%
  add_row(.,var = 'SATFAR_ENG', desc = "Saturated fat away from home",label= NA, value = NA)  %>%
  add_row(.,var = 'HEI_INDEX', desc = "Healthy eating index away from home",label= NA, value = NA)  %>%
  
  mutate(var = paste0(var,"_FAFH"))
...
h.sumfinal <- h.sum %>%
  ...
  mutate_if(is.numeric, convertInteger) %>%
  mutate_if(is.double, cleanNumeric, tol = 0.001) %>%
  labelled::set_variable_labels(.labels = setNames(as.list(safeCharacters(codebook_fafh$desc)), codebook_fafh$var), .strict = FALSE) %>%  # Set descriptions for codebook variables
  # labelled::set_variable_labels(.labels = setNames(as.list(paste(gvars, "geographic concordance")), gvars)) %>%  # Set descriptions for geo identifiers
  rename(
    hid = HHNUM#,  # Rename ID and weight variables to standardized names
    # weight = HHWGT
  ...
  rename_with(tolower) %>% 
  rename_with(~ if_else(. == "hid", ., paste0(., "_fafh")), everything()) %>%
...
#rm(d)
# Save data to disk (.fst)
fst::write_fst(x = h.sumfinal, path = "survey-processed/FAPS/2013/FAPS_2013_sumFAFHN_processed.fst", compress = 100)
fst::write_fst(x = codebook_fafh, path = "survey-processed/FAPS/2013/FAPS_2013_sumFAFHN_codebook.fst", compress = 100)
