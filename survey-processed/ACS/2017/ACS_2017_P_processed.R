...
# Unzip raw .zip file
unzip("survey-raw/ACS/2017/csv_pus.zip", exdir = tempdir(), overwrite = TRUE)
...
d <- d %>%
  ...
  select(acs_2017_hid, pid, weight, everything(), -starts_with("rep_"), starts_with("rep_")) %>%   # Reorder columns with replicate weights at the end
  arrange(acs_2017_hid, pid)
...
dictionary <- createDictionary(data = d, survey = "ACS", vintage = 2017, respondent = "P")
...
fst::write_fst(x = d, path = "survey-processed/ACS/2017/ACS_2017_P_processed.fst", compress = 100)
