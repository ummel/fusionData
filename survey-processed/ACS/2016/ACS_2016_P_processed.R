...
# Unzip raw .zip file
unzip("survey-raw/ACS/2016/csv_pus.zip", exdir = tempdir(), overwrite = TRUE)
...
d <- d %>%
  ...
  select(acs_2016_hid, pid, weight, everything(), -starts_with("rep_"), starts_with("rep_")) %>%   # Reorder columns with replicate weights at the end
  arrange(acs_2016_hid, pid)
...
dictionary <- createDictionary(data = d, survey = "ACS", vintage = 2016, respondent = "P")
saveRDS(object = dictionary, file = "survey-processed/ACS/2016/ACS_2016_P_dictionary.rds")
gc()

#----------------

# Save data to disk (.fst)
fst::write_fst(x = d, path = "survey-processed/ACS/2016/ACS_2016_P_processed.fst", compress = 100)
