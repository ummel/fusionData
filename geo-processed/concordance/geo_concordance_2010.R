...
geocorr.file <- "geo-raw/concordance/geocorr2018/geocorr2018_2116808121.csv.zip"  # 2018
...
data(bg_centroids_2010, package = "fusionData")
...
recs15.climate <- readRDS("geo-processed/climate/climate_zones_processed.rds") %>%
  ...
  labelled::set_variable_labels(.labels = c("State code", "County code (2010)", "RECS 2015 Building America climate zone", "RECS 2015 IECC climate zone"))
...
recs20.climate <- readRDS("geo-processed/climate/climate_zones_processed.rds") %>%
  ...
  labelled::set_variable_labels(.labels = c("State code", "County code (2010)",  "RECS 2020 Building America climate zone", "RECS 2020 IECC climate zone"))
...
recs24.climate <- readRDS("geo-processed/climate/climate_zones_processed.rds") %>%
  ...
  select(state, county10, starts_with("recs24_")) %>%
  labelled::set_variable_labels(.labels = c("State code", "County code (2010)", "RECS 2024 Building America climate zone",  "RECS 2024 IECC climate zone"))
...
result <- geocorr %>%
  ...
  left_join(recs24.climate, by = c("state", "county10")) %>%
...
saveRDS(defs, file = "geo-processed/concordance/geo_concordance_definitions_2010.rds")

# Save output as .fst file
fst::write_fst(result, "geo-processed/concordance/geo_concordance_2010.fst", compress = 100)
