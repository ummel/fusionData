...
# Read 50 RECS fused implicates for electricity consumption 
...
for (i in 1:50) {
  ...
  comp_CA_01 <- recs.sim01_now %>%
    ...
    summarize(county_weight = sum(weight),  # county total household weight
              ...
              .groups = "drop") 
  trimws(comp_CA_01) 
  comp_CA_01 <- filter(comp_CA_01, (state != "NA"))
  comp_CA_01$county <- tolower(comp_CA_01$county) 
  Newname <- paste("comp_CA_01_",i,sep="")
  
     assign(Newname, comp_CA_01)
  }

for (i in 1:50) {
  comp_CA <- get(paste0("comp_CA_01_",i,sep=""))
comp_CA <- filter(comp_CA, (state != "NA"))
#Count number of shared counties per PUMA
comp_CA$count <- str_count(comp_CA$county,'&')

comp_CA_sub0 <- filter(comp_CA, (count == "0"))
...
Newname1 <- paste("comp_CA_sub0_",i,sep="")
Newname2 <- paste("comp_CA_sub_",i,sep="")

assign(Newname1, comp_CA_sub0)
assign(Newname2, comp_CA_sub)
}

#Split each combined entry to separate dataframe

for (i in 1:50) {
  comp_CA_sub <- get(paste0("comp_CA_sub_",i,sep=""))
  ...
  for(j in 1:n) {
  get(paste("comp_CA_N",j,sep="")) -> comp
  ...
  Newname <- paste("comp_CA_small",j,sep="")
...
CA_new_list <- list()
for(k in 1:n){
  df.now <- get(paste0("comp_CA_small",k,sep=""))
  CA_new_list[[k]] <- df.now
}

CA_list <- do.call(rbind,CA_new_list) 
Newname1 <- paste("CA_list_",i,sep="")
assign(Newname1, CA_list)
...
for(i in 1:50) {
  get(paste("CA_list_",i,sep="")) -> CA_list
  get(paste("comp_CA_sub0_",i,sep="")) -> comp_CA_sub0
  comp_CA_new <- rbind(CA_list,comp_CA_sub0)
  
  Newname <- paste("comp_CA_new_",i,sep="")
  assign(Newname,comp_CA_new)
}


for(i in 1:50) {
  get(paste("comp_CA_new_",i,sep="")) -> comp_CA_new
  comp_CA <- comp_CA_new  %>%
  ...
  Newname <- paste("eval_CA_",i,sep="")
  assign(Newname,eval_CA)
}

#----------------------------------------------------------------------------------------------------
# Extract totalkwh from each implicate set for std calculation 

for(i in 1:50) {
  get(paste("eval_CA_",i,sep="")) -> eval_CA
  eval_CA_imp_t <- select(eval_CA, c('totalkwh.sim','county'))

    Newname <- paste("eval_CA_imp_t_",i,sep="")
  assign(Newname,eval_CA_imp_t)
}

#Merge all the extracted files into one for average calculation

Check_1 <-c() 
Ds <-eval_CA_imp_t_1

for(i in 2:50){
  get(paste("eval_CA_imp_t_",i,sep="")) -> eval_CA_x
  Ds<-merge(Ds,eval_CA_x, by ='county') 
  names(Ds)[names(Ds) == 'totalkwh.sim.x'] <- 'totalkwh.sim'
  names(Ds)[names(Ds) == 'totalkwh.sim.y'] <- (paste("imp_2_",i,sep=""))
  # Ds<- Ds[, !duplicated(colnames(Ds))]
  Newname <- paste("Check_",i,sep="")
  assign(Newname,Ds)
}

names(Check_50)[names(Check_50) == 'totalkwh.sim'] <- 'imp_2_50'

Check_50$totalkwh.sim_avg <- apply(Check_50[,2:51], 1, mean)
Check_50$totalkwh.sim_std  <- apply(Check_50[,2:51], 1, sd)

Check_51 <- select(Check_50, c('county','totalkwh.sim_avg','totalkwh.sim_std'))

# Aggregating PUMS county information for CA,MA,and NY 
fst::write_fst(Check_51, "production/validation/totalkwh_sim_avg_CA.fst", compress = 100)
...
for(i in 1:50) {
  ...
  Newname <- paste("eval_CA_2_",i,sep="")
  assign(Newname,eval_CA_2)
}

# Extract averagekwh.sim from each implicate set for std calculation 

for(i in 1:50) {
  get(paste("eval_CA_2_",i,sep="")) -> eval_CA
  eval_CA_imp_a <- select(eval_CA, c('averagekwh.sim','county'))
  
  Newname <- paste("eval_CA_imp_a_",i,sep="")
  assign(Newname,eval_CA_imp_a)
}
...
for(i in 2:50){
  get(paste("eval_CA_imp_a_",i,sep="")) -> eval_CA_x
  Ds<-merge(Ds,eval_CA_x, by ='county') 
...
# Aggregating PUMS county information for CA,MA,and NY 
fst::write_fst(Check_51, "production/validation/averagekwh_sim_avg_CA.fst", compress = 100)
...
for(i in 1:50) {
  get(paste("eval_CA_2_",i,sep="")) -> eval_CA
  rank.sim_a <- rank(-eval_CA$averagekwh.sim)
  rank.util_a <- rank(-eval_CA$averagekwh)
  eval_CA <- cbind (eval_CA, rank.sim_a,rank.util_a)
  Newname <- paste("eval_CA_2_",i,sep="")
  assign(Newname,eval_CA)
}
...
for(i in 1:50) {
  get(paste("eval_CA_2_",i,sep="")) -> eval_CA
  eval_CA_imp <- select(eval_CA, c('rank.sim_a','county'))
  paste("eval_CA_imp_2",i,sep="")
  
  Newname <- paste("eval_CA_imp_2_",i,sep="")
  assign(Newname,eval_CA_imp)
}
...
for(i in 2:50){
  get(paste("eval_CA_imp_2_",i,sep="")) -> eval_CA_x
  Ds<-merge(Ds,eval_CA_x, by ='county') 
...
# Aggregating PUMS county information for CA,MA,and NY 
fst::write_fst(Check_51, "production/validation/rank_sim_a_avg_CA.fst", compress = 100)
