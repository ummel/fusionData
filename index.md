...
- [Prepare fusion inputs](#prepare-fusion-inputs)
- [Generate fusion outputs](#generate-fusion-outputs)
...
fusionData includes a convenience function called
[`installPackage()`](https://ummel.github.io/fusionData/reference/installPackage.md)
to safely update (re-install) your local installation of the fusionData
...
reflect the changes until you pull and re-install.
[`installPackage()`](https://ummel.github.io/fusionData/reference/installPackage.md)
does a number of operations that our initial
...
Package-wide `.rda` data files. Loadable via
[`data()`](https://rdrr.io/r/utils/data.html), as usual for R packages.
*Most users do not need to access/modify this directory.*
...
Users will call these functions to perform fusion but typically don’t
need to access/modify the directory itself. Almost all fusion
input/output files are stored remotely given their size. The only files
that are pushed to the Github repository are .txt log files that contain
...
to assign variable descriptions (ideally, taken from the official
codebook) for each column. The script must then call the
[`createDictionary()`](https://ummel.github.io/fusionData/reference/createDictionary.md)
function to create and save a standardized “dictionary.rds” file.
[`createDictionary()`](https://ummel.github.io/fusionData/reference/createDictionary.md)
...
this at the end of
[RECS_2015_H_processed.R](https://github.com/ummel/fusionData/blob/master/survey-processed/RECS/2015/RECS_2015_H_processed.R):
...
function to compile all of fusionData’s individual survey dictionaries
into a single “universal” dictionary. The usage is straightforward:
...
As the console output reveals,
[`compileDictionary()`](https://ummel.github.io/fusionData/reference/compileDictionary.md)
updates two files: `data/dictionary.rda` and `data/surveys.rda`. These
files are part of the Github repository and are used by both the
“Universal Survey Dictionary” and “Survey Harmonization Tool” Shiny apps
that are part of fusionData.

If this is the first time you are calling
[`compileDictionary()`](https://ummel.github.io/fusionData/reference/compileDictionary.md)
for a new survey, you will need to add the survey abbreviation and
description of the survey to the R script `universe/app.R` on line 32 so
it can be called on by
[`universe()`](https://ummel.github.io/fusionData/reference/universe.md)
(described below).
...
At present, the
[`harmony()`](https://ummel.github.io/fusionData/reference/harmony.md)
app only allows specification of harmonies between a non-ACS donor
...
field is passed as-is to a dplyr::mutate() call within
...
the “Household aggregator” field tells
[`harmonize()`](https://ummel.github.io/fusionData/reference/harmonize.md)
...
Household aggregator field to “reference” we instruct
[`harmonize()`](https://ummel.github.io/fusionData/reference/harmonize.md)
...
field is the set to “sum” to instruct
[`harmonize()`](https://ummel.github.io/fusionData/reference/harmonize.md)
...
heating fuel variables. This is precisely what the
[`harmonize()`](https://ummel.github.io/fusionData/reference/harmonize.md)
...
Using the
[`harmony()`](https://ummel.github.io/fusionData/reference/harmony.md)
app to manually define harmonies and then letting
[`harmonize()`](https://ummel.github.io/fusionData/reference/harmonize.md)
take care of subsequent data manipulation makes the construction of
harmonized microdata easier, faster, and *much* safer. It is generally
advisable to use the
[`harmonize()`](https://ummel.github.io/fusionData/reference/harmonize.md)
app to create and edit harmonies. It is also possible to manually edit
the .R harmony files, if necessary, but be careful.
...
spatial data files is quite relaxed. The function
...
run
[`compileSpatial()`](https://ummel.github.io/fusionData/reference/compileSpatial.md)
to update the `geo_predictors.fst` file.
...
and
[`fusionOutput()`](https://ummel.github.io/fusionData/reference/fusionOutput.md)
functions, respectively. These functions safely execute all of the steps
required for successful fusion. In practice, users need only call the
two high-level functions and ensure that the resulting console output
and log files look good.
...
Now let’s make a call to
[`fusionInput()`](https://ummel.github.io/fusionData/reference/fusionInput.md).
The function arguments are fully documented in
[`?fusionInput`](https://ummel.github.io/fusionData/reference/fusionInput.md).
If you run this code in an interactive session, you will be prompted at
the console to approve certain steps before proceeding (again, this is
all documented). The console output is designed to be very informative
and describe what is being done behind the scenes.
...
and
[`fusionOutput()`](https://ummel.github.io/fusionData/reference/fusionOutput.md)
can be run in “test mode” – in fact, this is the default behavior. When
in test mode, result files are saved within the `/fusion_` directory to
prevent any conflict with (or overwrite) of production data in `/fusion`
(no underscore).
[`fusionInput()`](https://ummel.github.io/fusionData/reference/fusionInput.md)
returns the path to the directory where files are saved. We can confirm
that the path used `/fusion_` as expected:
...
Now let’s see the names of the files created by
[`fusionInput()`](https://ummel.github.io/fusionData/reference/fusionInput.md):
...
Every call to
[`fusionInput()`](https://ummel.github.io/fusionData/reference/fusionInput.md)
generates the same four generic result files:
...
It is usually necessary for a user to run
[`fusionInput()`](https://ummel.github.io/fusionData/reference/fusionInput.md)
more than once for a given donor survey, since the process can flag
issues with variable harmonization that need to be addressed before
finalizing the fusion inputs.

### Person-to-household aggregation

If `donor` refers to a survey with both household- and person-level
microdata *and* `respondent = "household"` *and* `fuse` includes
person-level variables, then we have a situation where person-level
fusion variables need to be aggregated to household-level prior to
fusion. For example, most variables in the ASEC 2019 microdata are at
the person level. When fusing with ACS 2019 data, these variables need
to be aggregated to the household level.

This is done automatically within
[`assemble`](https://ummel.github.io/fusionData/reference/assemble.html).
In this scenario, person-level fusion variables are aggregated based on
their class. By default, numeric variables return the household total
(sum), unordered factors return the level of the household’s reference
person, and ordered factors return the household’s maximum level. This
is one reason why specifying variables as ordered or unordered factors
is important within the ingestion process.

If the default aggregation methods are not correct for a specific
variable, then you can override them in one of two ways: using the
`agg_fun` argument or the `agg_adj` argument. Lets look at an example
using ASEC and two variables at the person level: `kidcneed`, which
flags if a child under the age of 14 needs paid childcare while their
parents work, and `schllunch`, which is the value of school lunch meals
provided to children for free at school.

First, download the processed microdata for both the ASEC 2019 and ACS
2019 surveys:

    getSurveyProcessed(survey = "ACS_2019")
    getSurveyProcessed(survey = "ACES_2019")

`{r, echo = TRUE} asec <- fst::read_fst("survey-processed/ASEC/2019/ASEC_2019_P_processed.fst") head(select(asec, asec_2019_hid, weight, rep_1, schllunch, kidcneed))`

By default, `schllunch` would be summed across all household members
(because it is numeric) and `kidcneed` would take the reference person’s
value (because it is an unordered factor). However, (somewhat
confusingly) in the person-level data, ASEC records `schllunch` as the
same for all members of the *family*. Summing `schllunch` across
household members would result in double counting the total school lunch
value of that household, even if there are multiple families in the
household. This problem occurs with many of the ASEC variables related
to poverty because the *family-level* response is recorded for each
*individual* in the family.

The default behavior for `kidcneed` would be the value of the reference
person. As the reference person is the householder, who is never a child
under the age 14, this would result in the aggregated variable being
always “not in universe”. Instead, we want to take the modal value
across members of the household. This will ignore the NIU values and be
a 1 if the majority of children need paid child care, and 0 if not.

Let’s implement these two custom adjustments to the aggregation process
within
[`fusionInput()`](https://ummel.github.io/fusionData/reference/fusionInput.md).

\`\`\`{r, echo = TRUE}

input.dir \<- fusionInput(donor = “ASEC_2019”, recipient = “ACS_2019”,
respondent = “household”, \# variables we always want as predictors:
force = c(“hhincome”, “race”, “educ”, “numprec”, “hhtenure”, “state”),
\# the two person-level variables we want to aggregate and fuse fuse =
c(“kidcneed”, “schllunch”), \# here we provide a list of specific custom
functions for aggregation agg_adj = list( schllunch =
~if.else(duplicated(data.table(asec_2019_hid, famid)), 0, schllunch) ),
\# here we provide a list of specific pre-packaged functions for
aggregation (which still override the defaults) agg_fun = list( kidcneed
= “mode” ), note = “ASEC example of custom aggregation”)
...
Every call to
[`fusionOutput()`](https://ummel.github.io/fusionData/reference/fusionOutput.md)
generates the same three generic result files:
...
and
[`fusionOutput()`](https://ummel.github.io/fusionData/reference/fusionOutput.md)
are “passing” start-to-finish in test mode before trying to your final
(and more time-consuming) fusion.
...
access
[`fusionOutput()`](https://ummel.github.io/fusionData/reference/fusionOutput.md)
is overkill (and a pain), a copy of
[`fusionOutput()`](https://ummel.github.io/fusionData/reference/fusionOutput.md)
is silently exported in the fusion*Model* package. This allows someone
to install and load only the fusionModel package in a server environment
and then use
[`fusionOutput()`](https://ummel.github.io/fusionData/reference/fusionOutput.md)
to do the fusion step – assuming the server has access to the necessary
input files.
